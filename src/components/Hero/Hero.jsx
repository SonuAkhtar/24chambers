// import CSS
import "./hero.css";

const Hero = () => {
  return (
    <section className="hero_section">
      <div className="hero_wrapper">
        <h1 className="hero_title one">
          24 Chambers: Your Trusted Legal Partner
        </h1>
        <h1 className="hero_title two">
          24 Chambers: Your Trusted Legal Partner
        </h1>
        <h1 className="hero_title three">
          24 Chambers: Your Trusted Legal Partner
        </h1>
        <h1 className="hero_title four">
          24 Chambers: Your Trusted Legal Partner
        </h1>
        <h1 className="hero_title five">
          24 Chambers: Your Trusted Legal Partner
        </h1>
        <h1 className="hero_title six">
          24 Chambers: Your Trusted Legal Partner
        </h1>
        <h1 className="hero_title seven">
          24 Chambers: Your Trusted Legal Partner
        </h1>
        <h1 className="hero_title eight">
          24 Chambers: Your Trusted Legal Partner
        </h1>
        <h1 className="hero_title nine">
          24 Chambers: Your Trusted Legal Partner
        </h1>
        <div className="hero_subtitle">
          <p>
            A law firm dedicated to delivering exceptional legal expertise and
            personalized service to clients. With a team of seasoned lawyers, 24
            Chambers provides tailored solutions backed by extensive knowledge
            and experience, ensuring clients receive the best possible
            representation.
          </p>

          <p>
            With the Head Office in New Delhi and a commitment to serving
            clients across India, 24 Chambers is well-positioned to navigate the
            legal landscape.
          </p>
        </div>
      </div>
    </section>
  );
};

export default Hero;
